...
>
<a href="/article/{{ $blog['slug'] }}" class="font-bold text-xl duration-300 group-hover:text-white group-hover:z-[5] hover:underline" >
    {{ $blog->title }}

</a>
  <div class="text-base text-gray-500 group-hover:text-white group-hover:z-[5]">
    Dibuat Oleh
    <a href="#" class="hover:underline group-hover:text-white">{{ $blog->author }}</a> 
    |
    <a href="" class="hover:underline group-hover:text-white">{{ $blog->category->name }}</a> | {{ $blog->created_at->diffForHumans() }}
    <p class="my-4 font-light group-hover:text-white">{{ Str::limit($blog['content'], 200) }}</p>
  </div>

  <a
    href="/article/{{ $blog['slug'] }}" class="text-[#1D2825] group-hover:z-[5] font-medium duration-300 group-hover:text-white mt-auto flex items-center gap-2 text-sm xl:text-base hover:underline"
  >
    Baca Selengkapnya
    <svg
      class="w-4 h-4"
