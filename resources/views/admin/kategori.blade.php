<x-layout-admin>
  ...
  <div class="relative overflow-x-auto">
    <div class="flex justify-end mb-4">
      <a href="#" id="addButton" class="inline-flex items-center bg-blue-500 hover:bg-blue-700 mt-5 text-white font-bold py-2 px-4 rounded">
      ...
      <tbody>
        ...
        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
          ...
          <td class="px-6 py-4">
            ...
            <form action="{{ route('kategori.destroy', $k->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus Kategori ini?');">
              @csrf
              @method('DELETE')
...
.then(data => {
  if (data.success) {
    Swal.fire('Kategori Ditambahkan', 'Nama kategori baru: ' + result.value, 'success')
    .then(() => {
      location.reload(); // Reload the page to see the updated data
    });
  } else {
    Swal.fire('Gagal', 'Kategori tidak berhasil ditambahkan', 'error');
  }
})
.catch(error => {
